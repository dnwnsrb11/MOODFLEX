...
Vue.js를 활용하면서 프론트엔드 개발의 전반적인 과정을 깊이 있게 이해할 수 있었습니다. 컴포넌트 구조를 학습하고, 받은 데이터를 효과적으로 활용하는 방법을 익혔습니다. 특히 동적인 데이터 바인딩과 상태 관리의 중요성을 실제 프로젝트를 통해 체감할 수 있었습니다.
...
특히 기억이 남는것은 다음과 같습니다.
    1. slick을 활용한 슬라이드 활용 
        ->  slick은 jQuery 인데 이를 vue와 사용할 때 큰 문제가 발생하였습니다. 
        slick의 공식문서처럼 vue에서 활용시 큰 문제가 발생했습니다 -> 원인은 간단히 다음과 같습니다.
        Dom이 전부 생성되기 전에 실행되기 때문에 오류가 발생 -> omMounut 를 활용하여 이를 방지하는 코드를 통하여 vue에서도 slick을 사용하도록 구성하였습니다. 
    2. 실시간 정보 수정
        -> axios를 활용하여 정보를 const = ref() 에 담아 출력하는 구성을 하였습니다. 실시간을 사용자에게 사용자의 행동을 시각적으로 출력을 하고자 실시간으로 ref에 추가 삭제를를 통하여 사용자가 마치 실시간으로 데이터의 변화를 확인할수 있도록 구성하였습니다.

이번 프로젝트를 통하여 더 다양하고 복잡한 프로젝트에 도전하고 싶습니다. 특히 Vue와 같은 프론트엔드 프레임워크를 처음 사용한 프로젝트 였으며 사용하며넛 프레임워크의 필요성을 크게 알아가는 과정이었습니다. 
또한 협업에 대한 경험을 크게 가질수 있었습니다. 단 두명이서 진행한 프로젝트 이지만 git으로 인한 많은 이슈가 발생하였습니다. 가장 대표적인것이 Merge 입니다. 각자 개인적인 시간에서 작업을 하고 git을 활용하여 
통합할때 항상 이슈가 발생하였습니다. 이번 프로젝트를 통하여 이를 해결하는 방법을 많이 배웠습니다.
    - git merge를 많이 경험할 수 있었습니다.
    - git push를 하기 전에 반드시 pull을 받아서 최신 상태를 유지하느 과정이 필요한것을 배웠습니다
